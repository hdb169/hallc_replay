...
# Name of the pedestal monitoring file for trigger sum
reportMonFile="reportMonitor_${spec}_${runNum}_${numEvents}.txt"
pedMonFile="pedReport_${spec}_${runNum}_${numEvents}.txt"

#Name of the pedestal monitoring file for detectors
pedMonFileDetc="pedReportDetc_${spec}_${runNum}_${numEvents}.txt"
...
runPedMonDetc="root -l -q \"${pedMonDir}/${spec}_detectors_ped.C(${runNum}, ${numEvents})|\""
...
outFilePedDetc="outDET${SPEC}.txt"
...
# Start analysis and monitoring plots.
{
  ...
  echo ""
  echo "Performing pedestal monitoring for ${SPEC} Detectors run ${runNum}."
  echo ":=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:="

  sleep 2
  eval ${runPedMon}
  mv "${outFilePed}" "${pedMonOutDir}/${pedMonFile}"
  mv "${outFilePedDetc}" "${pedMonOutDir}/{pedMonFile}"
